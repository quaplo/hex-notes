...
final class Version20250628195849 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        SQL);
        $this->addSql(<<<'SQL'
            DROP INDEX idx_77becee481c5f0b9
        SQL);
        $this->addSql(<<<'SQL'
            DROP INDEX idx_77becee49019388a
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project DROP CONSTRAINT user_project_pkey
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project ADD role VARCHAR(50) NOT NULL
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project ADD project_id UUID NOT NULL
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project ADD user_id UUID NOT NULL
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project DROP project_entity_id
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project DROP user_entity_id
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project ADD CONSTRAINT FK_77BECEE4166D1F9C FOREIGN KEY (project_id) REFERENCES projects (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project ADD CONSTRAINT FK_77BECEE4A76ED395 FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE
        SQL);
        $this->addSql(<<<'SQL'
            CREATE INDEX IDX_77BECEE4166D1F9C ON user_project (project_id)
        SQL);
        $this->addSql(<<<'SQL'
            CREATE INDEX IDX_77BECEE4A76ED395 ON user_project (user_id)
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE user_project ADD PRIMARY KEY (project_id, user_id)
        SQL);
    }
