...
describe('ProjectName Value Object', function () {
    
    test('project name can be created with valid string', function () {
        $name = new ProjectName('Valid Project Name');
        
        expect((string)$name)->toBe('Valid Project Name');
    });

    test('project name throws exception for empty string', function () {
        expect(fn() => new ProjectName(''))
            ->toThrow(InvalidArgumentException::class, 'Project name cannot be empty');
    });

    test('project name throws exception for whitespace only string', function () {
        expect(fn() => new ProjectName('   '))
            ->toThrow(InvalidArgumentException::class, 'Project name cannot be empty');
    });

    test('project names are equal when values match', function () {
    ...
    test('project names are not equal when values differ', function () {
    ...
    test('project name accepts minimum valid length', function () {
        $name = new ProjectName('A'); // Minimum 1 znak
        
        expect((string)$name)->toBe('A');
    });

    test('project name accepts long string', function () {
    ...
    test('project name supports unicode characters', function () {
